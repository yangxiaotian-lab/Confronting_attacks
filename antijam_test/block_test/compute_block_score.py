...
if __name__ == '__main__':
    ...
    block_para = np.linspace(0, 30, 5).astype('int')
    reco_ac = []
    reco_loss = []
    # 读数据集列表 循环数据集
    test_case_dirs = '../../../output/antijam_data/block'
    dir_list = sorted(os.listdir(test_case_dirs), key=lambda str: (str[:5], int(str[6:])))
    no = 1
    for dir in dir_list:
    ...
    # print(reco_ac)
    # print(reco_loss)
    deltaAccRise = -np.mean(np.diff(reco_ac) / np.ones(len(block_para) - 1))
    deltaLossRise = np.mean(np.diff(reco_loss) / np.ones(len(block_para) - 1))
    print("准确率下降%.2f%%/遮挡面积宽高增加3像素\t" % (deltaAccRise * 100))
    print("损失值增大%.4f/遮挡面积宽高增加3像素" % deltaLossRise)

    # 写入数据
    block_ac_score = round(((math.pow(100000, 1 - deltaAccRise) - 1) / (100000 - 1)) * 100, 2)      # 将准确率(1 - deltaAccRise)放大
    # print(block_ac_score)
    # block_ac_score = round(100 - deltaAccRise * 100, 2)
    block_loss_score = round(100 - deltaLossRise * 100, 2)
    ans_dict = {"block_ac_score": block_ac_score, "block_loss_score": block_loss_score}
