...
if __name__ == '__main__':
    ...
    brightness_para = [1.2, 1.3, 1.4, 1.5, 1.6, 1.7]
    reco_ac = []
    reco_loss = []
    # 读数据集列表 循环数据集
    test_case_dirs = '../../../output/antijam_data/brightness'
    dir_list = sorted(os.listdir(test_case_dirs), key=lambda str:(str[:10], float(str[11:])))
    no = 1
    for dir in dir_list:
    ...
    # print(reco_ac)
    # print(reco_loss)
    deltaAccRise = -np.mean(np.diff(reco_ac) / np.ones(len(brightness_para) - 1))
    deltaLossRise = np.mean(np.diff(reco_loss) / np.ones(len(brightness_para) - 1))
    print("准确率下降%.2f%%/光照增加20%%\t" % (deltaAccRise * 100))
    print("损失值增大%.4f/光照增加20%%" % deltaLossRise)

    # 写入数据
    brightness_ac_score = round(((math.pow(100000, 1 - deltaAccRise) - 1) / (100000 - 1)) * 100, 2)     # round(100 - deltaAccRise * 100, 2)
    brightness_loss_score = round(100 - deltaLossRise * 100, 2)
    ans_dict = {"brightness_ac_score": brightness_ac_score, "brightness_loss_score": brightness_loss_score}
