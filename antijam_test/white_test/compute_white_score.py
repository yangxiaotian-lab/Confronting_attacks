...
if __name__ == '__main__':
    ...
    white_para = np.linspace(1, 4, 4).astype('int')
    reco_ac = []
    reco_loss = []
    # 读数据集列表 循环数据集
    test_case_dirs = '../../../output/antijam_data/white'
    ...
    # print(reco_ac)
    # print(reco_loss)
    deltaAccRise = -np.mean(np.diff(reco_ac) / np.ones(len(white_para) - 1))
    deltaLossRise = np.mean(np.diff(reco_loss) / np.ones(len(white_para) - 1))
    print("准确率下降%.2f%%/美白程度增加10%%\t" % (deltaAccRise * 100))
    print("损失值增大%.4f/美白程度增加10%%" % (deltaLossRise))

    # 写入数据
    white_ac_score = round(((math.pow(100000, 1 - deltaAccRise) - 1) / (100000 - 1)) * 100, 2)  # round(100 - deltaAccRise * 100, 2)
    white_loss_score = round(100 - deltaLossRise * 100, 2)
    ans_dict = {"white_ac_score": white_ac_score, "white_loss_score": white_loss_score}
