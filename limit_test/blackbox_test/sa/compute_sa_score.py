...
if __name__ == '__main__':
    ...
    sa_para = np.linspace(1, 10, 2).astype('double')
    reco_ac = []
    reco_loss = []
    # 读数据集列表 循环数据集
    test_case_dirs = '../../../../output/limit_data/blackbox_data/sa'
    ...
    # print(reco_ac)
    # print(reco_loss)
    deltaAccRise = -np.mean(np.diff(reco_ac) / np.diff(sa_para))
    deltaLossRise = np.mean(np.diff(reco_loss) / np.diff(sa_para))
    print("准确率下降%.2f%%/扰动增加10%%\t" % (deltaAccRise * 100))
    print("损失值增大%.4f/扰动增加10%%" % (deltaLossRise))

    # 写入数据
    sa_ac_score = round(((math.pow(100000, 1 - deltaAccRise) - 1) / (100000 - 1)) * 100, 2)
    sa_loss_score = round(100 - deltaLossRise * 100, 2)
    ans_dict = {"sa_ac_score": sa_ac_score, "sa_loss_score": sa_loss_score}
