...
if __name__ == '__main__':
    ...
    reco_loss = []
    # 读数据集列表 循环数据集
    test_case_dirs = '../../../../output/limit_data/whitebox_data/cw'
    dir_list = sorted(os.listdir(test_case_dirs), key=lambda str:(str[:2], float(str[3:])))
    no = 0
    for dir in dir_list:
    ...
    # print(reco_ac)
    # print(reco_loss)
    deltaAccRise = np.mean(np.diff(reco_ac) / np.diff(cw_para))
    deltaLossRise = np.mean(np.diff(reco_loss) / np.diff(cw_para))
    print("准确率上升%.2f%%/优化系数增加10%%\t" % (deltaAccRise * 100))
    print("损失值增大%.4f/优化系数增加10%%" % (deltaLossRise))

    # 写入数据
    cw_ac_score = round(((math.pow(100000, 1 - deltaAccRise) - 1) / (100000 - 1)) * 100, 2)
    cw_loss_score = round(100 - deltaLossRise * 100, 2)
    ans_dict = {"cw_ac_score": cw_ac_score, "cw_loss_score": cw_loss_score}
