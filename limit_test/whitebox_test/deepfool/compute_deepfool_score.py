...
if __name__ == '__main__':
    ...
    reco_loss = []
    # 读数据集列表 循环数据集
    test_case_dirs = '../../../../output/limit_data/whitebox_data/deepfool'
    dir_list = sorted(os.listdir(test_case_dirs), key=lambda str:(str[:8], int(str[9:])))
    no = 1
    for dir in dir_list:
    ...
    # print(reco_ac)
    # print(reco_loss)
    deltaAccRise = -np.mean(np.diff(reco_ac) / np.diff(df_para))
    deltaLossRise = np.mean(np.diff(reco_loss) / np.diff(df_para))
    print("准确率下降%.2f%%/扰动增加10%%\t" % (deltaAccRise * 100))
    print("损失值增大%.4f/扰动增加10%%" % (deltaLossRise))

    # 写入数据
    df_ac_score = round(((math.pow(100000, 1 - deltaAccRise) - 1) / (100000 - 1)) * 100, 2)
    df_loss_score = round(100 - deltaLossRise * 100, 2)
    ans_dict = {"df_ac_score": df_ac_score, "df_loss_score": df_loss_score}
