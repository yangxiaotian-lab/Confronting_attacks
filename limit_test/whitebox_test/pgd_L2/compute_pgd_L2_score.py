...
if __name__ == '__main__':
    ...
    reco_loss = []
    # 读数据集列表 循环数据集
    test_case_dirs = '../../../../datalimit_data/whitebox_data/pgd_L2'
    ...
    # print(reco_ac)
    # print(reco_loss)
    deltaAccRise = -np.mean(np.diff(reco_ac) / np.diff(pgd_L2_para))
    deltaLossRise = np.mean(np.diff(reco_loss) / np.diff(pgd_L2_para))
    print("准确率下降%.2f%%/扰动增加10%%\t" % (deltaAccRise * 100))
    print("损失值增大%.4f/扰动增加10%%" % (deltaLossRise))

    # 写入数据
    pgd_L2_ac_score = round(((math.pow(100000, 1 - deltaAccRise) - 1) / (100000 - 1)) * 100, 2)
    pgd_L2_loss_score = round(100 - deltaLossRise * 100, 2)
    ans_dict = {"pgd_L2_ac_score": pgd_L2_ac_score, "pgd_L2_loss_score": pgd_L2_loss_score}
