...
class White_box_attack(object):
    ...
    def PGD_L2_attack(self, img_no, iters=100, eps=0.2 / 255, eps_max=6 / 255 * 2, L2_norm=10):
        ...
        for i in range(iters):
            ...
            img_noise = img_noise - eps * grad / torch.norm(grad, p=2)
            img_noise = torch.clamp(img_noise, -eps_max, eps_max)
            # img_noise = np.clip(img_noise.detach().cpu().numpy(), -eps_max, eps_max)  # Gradient Project
            img_attack = torch.clamp(img_noise + img_orign, -1, 1).detach()
...
if __name__ == "__main__":
    ...
    pgd_L2_para = np.linspace(1, 10, 10).astype('double')

    ori_dir = '../../../../dataset'
    print('加载的数据集为: ', ori_dir)
    img_no = 0
    dirlist = os.listdir(ori_dir)
    for value in pgd_L2_para:
        print("===================当前参数为: " + str(value) + " ===================")
        for dir in dirlist:
            ...
            for f in files:
                ...
                img_attack = White_box_attack(img_origin, net).PGD_L2_attack(img_no, eps=1 / 255 * value)

                img_attack = 127.5 * (img_attack.detach().cpu().numpy().transpose(1, 2, 0) + 1)
                img_attack = Image.fromarray(img_attack.astype('uint8'))

                save_folder = '../../../../output/limit_data/whitebox_data/pgd_L2/PGD_L2_' + str(value) + '/' + dir
                if not os.path.exists(save_folder):
                    os.makedirs(save_folder)
                save_path = save_folder + '/' + 'PGD_L2_' + str(value) + '+' + f[:-4] + '.png'
