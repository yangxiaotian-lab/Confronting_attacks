...
if __name__ == '__main__':
    ...
    reco_loss = []
    # 读数据集列表 循环数据集
    test_case_dirs = '../../../../output/limit_data/whitebox_data/pgd_∞'
    dir_list = sorted(os.listdir(test_case_dirs), key=lambda str:(str[:5], float(str[6:])))
    no = 1
    for dir in dir_list:
    ...
    # print(reco_ac)
    # print(reco_loss)
    deltaAccRise = -np.mean(np.diff(reco_ac) / np.diff(pgd_para))
    deltaLossRise = np.mean(np.diff(reco_loss) / np.diff(pgd_para))
    print("准确率下降%.2f%%/扰动增加10%%\t" % (deltaAccRise * 100))
    print("损失值增大%.4f/扰动增加10%%" % (deltaLossRise))

    # 写入数据
    pgd_Linf_ac_score = round(((math.pow(100000, 1 - deltaAccRise) - 1) / (100000 - 1)) * 100, 2)
    pgd_Linf_loss_score = round(100 - deltaLossRise * 100, 2)
    ans_dict = {"pgd_Linf_ac_score": pgd_Linf_ac_score, "pgd_Linf_loss_score": pgd_Linf_loss_score}
