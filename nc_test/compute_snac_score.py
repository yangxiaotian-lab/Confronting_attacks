    ...
    for f in files:
        ...
        for pic in pics:
            ...
            for key in dg.high.keys():
                ...
                neuron_num += len(dg.high[key])
            snac = UpperCornerNeuron / neuron_num
            print('img_no:' + str(img_no) + '\tSNAC: ' + str(snac))
            img_no += 1
    return snac
...
if __name__ == "__main__":
    ...
    snac = SNAC(dg, dir)  # SNAC都与[low, high]相关

    # 写入数据
    snac_score = round(snac * 100, 2)
    ans_dict = {"snac_score": snac_score}
